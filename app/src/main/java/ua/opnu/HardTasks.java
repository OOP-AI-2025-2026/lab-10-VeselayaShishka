...
public class HardTasks {
    ...
    public List<Order> getOrdersWithBabyProducts() {
        ...
        return orders.stream().filter(s ->
                s.getProducts().stream().anyMatch(p -> p.getCategory().equals("Baby"))).toList();
    }
    ...
    public List<Product> applyDiscountToToys() {
        ...
        return products.stream().
                ...
                peek(pr -> pr.setPrice(pr.getPrice()*0.5)).
                toList();
    }
    ...
    public Map<Integer, Integer> getOrdersProductsMap() {
        ...
        return orders.stream().collect(Collectors.toMap(
                pr -> pr.getId(), pr-> pr.getProducts().size()
        ));
    }
    ...
    public Map<String, List<Integer>> getProductsByCategory() {
        ...
        return products.stream().collect(Collectors.
                groupingBy(Product::getCategory,Collectors.
                        mapping(Product::getId, Collectors.toList())));
    }
